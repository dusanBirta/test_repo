...
# Function to enhance image with Real-ESRGAN
def enhance_image(input_image_path, output_image_path):
    enhance_command = f"/home/appuser/venv/bin/python Real-ESRGAN/inference_realesrgan.py -n RealESRGAN_x4plus -i {input_image_path} --outscale 3.5 --face_enhance"
    subprocess.run(enhance_command, shell=True)
...
if uploaded_file is not None:
    ...
    for result in results:
        ...
        for xyxy_box in xyxy_boxes:
            ...
            cropped_image_path = f'cropped_face_{face_counter}.jpg'
            cv2.imwrite(cropped_image_path, cropped_image)

            # Enhance the cropped face image
            enhanced_image_path = f'Real-ESRGAN/cropped_face_{face_counter}_out.jpg'
